...
function Decoder(bytes, port) {
  ...
  if(Ext!=0x0f)
  {
    decode.AirTemp=parseFloat(((bytes[2]<<24>>16 | bytes[3])/100).toFixed(2));
    decode.AirHumid=parseFloat((((bytes[4]<<8 | bytes[5])&0xFFF)/10).toFixed(1));
  }
  if(Connect=='1')
  {
  ...
  {
    meta.Work_mode="Interrupt Sensor send";
    decode.Exti_pin_level=bytes[7] ? "High":"Low";  
    decode.Exti_status=bytes[8] ? "True":"False";
  }
  else if(Ext=='5')
  {
    meta.Work_mode="Illumination Sensor";
    decode.Illumi=bytes[7]<<8 | bytes[8];
    
  }
  else if(Ext=='6')
  {
    meta.Work_mode="ADC Sensor";
    decode.ADC_V=(bytes[7]<<8 | bytes[8])/1000;
  }
  else if(Ext=='7')
  {
    meta.Work_mode="Interrupt Sensor count";
